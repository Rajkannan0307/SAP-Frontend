...
import { GetPlantStockListApi, FetchPlantStockApi } from "../controller/MfgBomApiService";
...
const PlantStock = () => {
  ...
  const columns = [
    ...
    { field: "quality_inspection_qty", headerName: "Quality Inspection Qty", width: 150, align: "center", headerAlign: "center" },
    { field: "blocked_qty", headerName: "Blocked Qty", width: 110, align: "center", headerAlign: "center" },
    {
      field: "stock_date",
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await GetPlantStockListApi();
      setOriginalRows(response || []);
      setRows(response || []);
    } catch (error) {
      console.error(error);
      toast.error("Failed to load Plant Stock data.");
      setOriginalRows([]);
      setRows([]);
    } finally {
      setLoading(false);
  ...
  const handleSearch = () => {
    ...
    const filteredRows = originalRows.filter((row) =>
      ["plant", "storage_location", "material_code", "material_desc"].some((key) => {
        const value = row[key];
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      toast.success(
        `Plant Stock fetched successfully. New snapshot inserted: ${result.inserted} row(s), previous batch deactivated: ${result.inactivated} row(s).`
      );
      await fetchData();
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        <SectionHeading>Plant Stock</SectionHeading>
      </div>
      ...
      >
        <DataGrid
          ...
          getRowId={(row) => row.plant_stock_id}
          disableSelectionOnClick
          loading={loading}
          columnHeaderHeight={35}
          rowHeight={35}
          slots={{ toolbar: CustomToolbar }}
          localeText={{ noRowsLabel: "No Plant Stock data found. Click \"Fetch\" to import the latest MB52 file." }}
          sx={{
