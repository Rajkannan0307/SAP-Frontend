...
import { AddOrEditProduct, AddOrEditProductMapping, getProductdetails, getProductMappingdetails } from "../../controller/PMPApiService";
...
const ProductMappingScreen = () => {
    ...
    const columns = [
        { field: "prod_mapping_id", headerName: "SI No", width: 80 },
        { field: "fg_prod_name", headerName: "FG Product", flex: 1 },
        { field: "sfg_prod_name", headerName: "SFG Product", flex: 1 },
        {
    ...
    const handleSearch = () => {
        ...
        const filteredRows = originalRows.filter((row) =>
            ["fg_prod_name", "sfg_prod_name"].some((key) => {
                const value = row[key];
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await getProductMappingdetails()
            console.log("getProductMappingdetails - ", response)
        ...
        >
            ...
            >
                <DataGrid
                    ...
                    getRowId={(row) => row.prod_mapping_id} // Specify a custom id field
...
const AddDialog = ({ open, setOpenAddModal, setRefreshData, editData }) => {
    ...
    const [fgProdList, setFgProdList] = useState([])
    ...
    const validationSchema = Yup.object({
        fg_prod_id: Yup.string()
            .required("FG Product is required"),
        sfg_prod_id: Yup.string()
            .required("SFG Product is required"),
    ...
    const formik = useFormik({
        initialValues: {
            fg_prod_id: editData?.fg_prod_id || "",
            sfg_prod_id: editData?.sfg_prod_id || "",
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                if (editData) {
                    const payload = {
                        prod_mapping_id: editData?.prod_mapping_id,
                        ...values,
                        user_id: userId
                    }
                    await AddOrEditProductMapping(payload)
    ...
    useEffect(() => {
        const fetchPropTypeData = async () => {
            const response1 = await getProductdetails("fg")
            const response2 = await getProductdetails("all")
            setFgProdList(response1)
            setAllProdList(response2)
        }

        if (open) fetchPropTypeData()
    }, [open])
    ...
    return (
        ...
        >
            <DialogTitle id="alert-dialog-title">
                {`${editData ? "Edit" : "Add"} Product Mapping`}
            </DialogTitle>
            ...
            <DialogContent sx={{ pb: 0 }}>
                <div style={{
                    ...
                    <TextField
                        ...
                        label="FG Product"
                        fullWidth
                        value={formik.values.fg_prod_id}
                        onChange={(e) => {
                            const selectedId = e.target.value;
                            formik.handleChange(e);
                        }}
                        onBlur={formik.handleBlur}
                        error={formik.touched.fg_prod_id && Boolean(formik.errors.fg_prod_id)}
                        helperText={formik.touched.fg_prod_id && formik.errors.fg_prod_id}
                        sx={{
                    ...
                    >
                        {fgProdList?.map((option, i) => (
                            <MenuItem sx={{ fontSize: 12 }} key={i} value={option.Prod_ID}>
                                {option.Name}
                            </MenuItem>
                        )) || []}
                    </TextField>
                    ...
                    <TextField
                        ...
                        label="SFG Product"
                        fullWidth
                        value={formik.values.sfg_prod_id}
                        onChange={(e) => {
                            const selectedId = e.target.value;
                            formik.handleChange(e);
                        }}
                        onBlur={formik.handleBlur}
                        error={formik.touched.sfg_prod_id && Boolean(formik.errors.sfg_prod_id)}
                        helperText={formik.touched.sfg_prod_id && formik.errors.sfg_prod_id}
                        sx={{
                            ...CommonMuiStyles.textFieldSmallSx2,
                            minWidth: 200,
                            mt: 1
...
export default ProductMappingScreen;