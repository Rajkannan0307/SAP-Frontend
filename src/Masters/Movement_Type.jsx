...
const Movement_Type = () => {
  ...
  const handleSearch = () => {
    ...
    if (!text) {
      ...
      const filteredRows = originalRows.filter((row) =>
        ["Movement_Code","Movement_Name"].some((key) => {
          const value = row[key];
  ...
  const handleRowClick = (params) => {
    ...
    setMovementName(params.row.Movement_Name);
    setMovementID(params.row.Movement_ID)
   ...
   if (MovementCode.toString().length !== 3) {
    ...
    try {
      ...
      if (response.data.success) {
        alert("Movement Type added successfully!");
        getData(); // refresh UI (e.g. user list)
        handleCloseAddModal(); // close the modal
      } else {
        alert(response.data.message || "Failed to add Movement Type.");
      }
    } catch (error) {
      console.error("Error in adding Movement Type:", error);
      ...
      if (
        ...
        alert("An error occurred while adding the Movement Type.");
      }
    }
  };
  ...
  const handleDownloadExcel = () => {
    ...
    const DataColumns = [
      "Movement_Code","Movement_Name","ActiveStatus"
    ];
    ...
    >
      ...
      >
        <DataGrid
          ...
          getRowId={(row) => row.Movement_ID} // Specify a custom id field
